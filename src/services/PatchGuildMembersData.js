    ...
    if (fullUpdate) {
        if (!character) {
            ...
            try {
                ...
                const updatedData = await fetchData(server, realm, name, undefined, isOlderThanDay);
                ...
                if (updatedData?.code && updatedData?.data?.blizID) {
                    ...
                    if (updatedData.code === 202 || updatedData.code === 409)
                        setter = updatedData?.data;
                } else {
...
try {
    ...
    // if (fullUpdate)
    console.info(
        `[PatchPvP] ${fullUpdate ? "Full " : ""}Update succeed: ${now.toLocaleDateString()} ${endNow.toLocaleTimeString()} `,
    );
